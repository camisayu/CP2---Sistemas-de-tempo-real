...
// Tag para logs
static const char *TAG = "Camila-RM:92958";
...
void init_task_watchdog(void)
{
    ...
    printf("{Camila-RM:92958} [WDT] Watchdog inicializado com timeout de %d segundos.\n", WDT_TIMEOUT_S);
}
...
void task_generate(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        if (sent == pdPASS)
        {
            printf("{Camila-RM:92958} [FILA] Dado %d enviado com sucesso!\n", value);
        }
        else
        {
            printf("{Camila-RM:92958} [FILA] Fila cheia, dado %d descartado!\n", value);
        }
...
void task_receive(void *pvParameters)
{
    ...
    while (1)
    {
        if (xQueueReceive(dataQueue, &receivedValue, pdMS_TO_TICKS(RECEPTION_TIMEOUT_MS)) == pdPASS)
        {
            ...
            {
                *ptr = receivedValue;
                printf("{Camila-RM:92958} [RECEPÇÃO] Valor recebido: %d\n", *ptr);
                free(ptr);
            }
            else
            {
                printf("{Camila-RM:92958} [RECEPÇÃO] Erro ao alocar memória!\n");
            }
        ...
        else
        {
            ...
            printf("{Camila-RM:92958} [RECEPÇÃO] Nenhum dado recebido no tempo limite!\n");
            printf("{Camila-RM:92958} [RECEPÇÃO] Tentando recuperar...\n");
            ...
            {
                printf("{Camila-RM:92958} [RECEPÇÃO] Recuperação bem-sucedida! Valor: %d\n", receivedValue);
                reception_ok = true;
            }
            else
            {
                printf("{Camila-RM:92958} [RECEPÇÃO] Falha persistente. Encerrando tarefa...\n");
                reception_ok = false;
                vTaskDelete(NULL);
            }
        }
...
void task_supervision(void *pvParameters)
{
    ...
    while (1)
    {
        printf("\n{Camila-RM:92958} [SUPERVISÃO] STATUS DO SISTEMA:\n");
...
void app_main(void)
{
    printf("{Camila-RM:92958} [SISTEMA] Inicializando sistema multitarefa...\n");
    ...
    if (dataQueue == NULL)
    {
        printf("{Camila-RM:92958} [ERRO] Falha ao criar fila!\n");
        return;
    }
    ...
    printf("{Camila-RM:92958} [SISTEMA] Tarefas iniciadas com sucesso!\n");
}